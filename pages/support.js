...
export default function SupportPage() {
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      // You can replace this with /api/support later
  ...
  return (
    <>
      <Head>
        <title>Support · Genio</title>
        <meta name="description" content="Get help with Genio and your AI Twin." />
      </Head>
      ...
      <style jsx>{`
        ...
        .hero h1 { font-size:40px; margin-bottom:10px; background:linear-gradient(90deg,#ffd54d,#ffae00); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        .hero p { color:#c9d6e5; }

        .grid { max-width:1100px; margin:24px auto; padding:0 20px; display:grid; gap:16px; grid-template-columns:repeat(3,minmax(0,1fr)); }
        @media (max-width:900px){ .grid{ grid-template-columns:1fr; } }
        .faq { background:#0f1b33; border:1px solid #22304a; border-radius:14px; padding:18px; }
        .faq h3 { margin:0 0 6px; }
        .faq p { margin:0; color:#b8c8df; }

        .formWrap { max-width:900px; margin:8px auto 32px; padding:0 20px; width:100%; }
        .form { background:#0f1b33; border:1px solid #22304a; border-radius:14px; padding:18px; }
        .form h2 { margin:0 0 12px; }
        .row { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px; }
        @media (max-width:780px){ .row{ grid-template-columns:1fr; } }
        .field { display:flex; flex-direction:column; margin-bottom:12px; }
        .field label { font-size:14px; margin-bottom:6px; color:#dbe7fb; }
        input, select, textarea {
          background:#0b1730; border:1px solid #2a3a56; color:#e6eef8; border-radius:12px; padding:12px 14px; outline:none;
        }
        input:focus, select:focus, textarea:focus { border-color:#3f86ff; box-shadow:0 0 0 3px #3f86ff33; }
        .note { background:#0f3020; border:1px solid #2e7a59; color:#b8ffde; border-radius:10px; padding:10px 12px; margin:8px 0; }
