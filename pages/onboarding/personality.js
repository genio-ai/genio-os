    ...
    <>
      ...
      <main className="container main">
        <section className="card" aria-labelledby="title">
          {/* Neon animated logo (always visible) */}
          <div className="logo">genio os</div>
          ...
          <div className="actions">
            <button
              type="button"
              className="btn outline ripple"
              onMouseDown={setRipplePoint}
              onTouchStart={setRipplePoint}
              onClick={goBack}
            >Go Back</button>
            ...
            <button
              ...
              onTouchStart={setRipplePoint}
              onClick={saveNext}
            >{busy ? "Saving…" : "Save & Continue — Voice"}</button>
          </div>
      ...
      <style jsx>{`
        ...
        /* Standalone page — no top gap */
        .main{
          padding:24px 0 70px; /* smaller top to remove any "header ghost" */
          background:
        ...
        /* Big breathing logo */
        .logo{
          font-weight:900; font-size:40px; margin-bottom:20px;
          background:linear-gradient(135deg,var(--neon1),var(--neon2));
          -webkit-background-clip:text; -webkit-text-fill-color:transparent;
          text-shadow:0 0 15px rgba(111,195,255,.65),0 0 30px rgba(32,227,178,.45);
        ...
        /* Glow pulse only when actionable */
        .pulse{ animation: ctaPulse 2.6s ease-in-out infinite; }

        /* Ripple using --x/--y from JS */
        .ripple::after{
          content:"";
        ...
        /* Keyframes */
        @keyframes pulseGlow {
          0%,100% { text-shadow:0 0 12px rgba(111,195,255,.55),0 0 26px rgba(32,227,178,.35); transform:scale(1); }
          50% { text-shadow:0 0 20px rgba(111,195,255,.85),0 0 36px rgba(32,227,178,.55); transform:scale(1.03); }
        }
        @keyframes ctaPulse {
        ...
        @media (prefers-reduced-motion: reduce){
          .logo, .pulse, .main { animation: none !important; }
