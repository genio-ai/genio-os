    ...
    <>
      ...
      <style jsx>{`
        :root{
          ...
          --danger:#FF4D4F; --danger-ink:#2a0f14;
        }
        .container{width:min(860px,92%); margin-inline:auto}
        a{color:inherit; text-decoration:none}

        /* Header: lifted with safe-area, clear border & shadow */
        .hdr{
        ...
        /* Main: add top spacing so content never sits under header */
        .main{padding:24px 0 56px; padding-top: clamp(24px, 24px + env(safe-area-inset-top), 48px);}

        .card{border:1px solid var(--stroke-soft);
          background:linear-gradient(180deg, rgba(15,23,37,.97), rgba(12,18,30,.97));
          border-radius:18px; padding:16px; box-shadow:var(--shadow)}
        .card-head h1{margin:0 0 6px; font-size:clamp(24px,5vw,36px);
          background:linear-gradient(180deg,#f4f8ff, #cfe0ff); -webkit-background-clip:text; color:transparent}
        ...
        .actions{
          ...
          background:linear-gradient(180deg, rgba(12,18,30,0), rgba(12,18,30,.88));
        ...
        .btn.neon:focus-visible{outline:3px solid rgba(111,195,255,.5); outline-offset:2px}
        .btn.outline{background:#101a2b; border:1px solid var(--stroke)}
        /* Strong red delete button */
        .btn.danger{
          border:1px solid #a11a1f;
          background:#331012;
          color:#ffecec;
        }
        .btn.danger:hover{background: #3a0e10;}
        .btn.danger:active{background:#2c0b0d}
        .btn.danger:focus-visible{outline:3px solid rgba(255,77,79,.6); outline-offset:2px}
        /* High-contrast pill */
        .manage .btn.danger{
          background: linear-gradient(180deg, #FF6A6C, #FF4D4F);
          color:#1f0b0c; border: none;
          box-shadow: 0 6px 14px rgba(255,77,79,.35);
        }
        .manage .btn.danger:hover{filter:brightness(.95)}
        .manage .btn.danger:active{filter:brightness(.9)}
