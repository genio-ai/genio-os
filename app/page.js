...
export default function Page() {
  ...
  function closeModal(){
    ...
    if (inst) {
      inst.teardown().catch(()=>{}).finally(()=>{ dropinInstanceRef.current=null; });
    }
  }
    ...
    if(!dropinInstanceRef.current){
      try{
        ...
        const inst = await braintree.dropin.create({
          ...
          card:{ cardholderName:true }, // CVV will show once enabled in Dashboard
          paypal:false,
          locale:"en_US"
        });
  ...
  return (
    <div className="page">
      ...
      {/* MODAL */}
      {showModal && (
        <div className="modalOverlay" role="dialog" aria-modal="true" aria-label="Payment" onClick={(e)=>{ if(e.target===e.currentTarget) closeModal(); }}>
          <div className="modal card">
            <h3 className="modalH">Donation • <span dir="rtl">الدفع</span></h3>

            {/* Keep payment fields LTR for clarity */}
            <div dir="ltr" className="dropinWrap">
              <div ref={dropinContainerRef} />
            </div>

            <div className="totalRow">
              <div className="totalLbl">Total</div>
              <div className="totalAmt">{fmt.format(total)}</div>
            </div>

            <div className="rowBtns">
              <button className="btnPrimary payBtn" onClick={confirmPay} disabled={busy}>
                {busy ? "Processing…" : "Pay securely • ادفع بأمان"}
              </button>
              <button className="btnLink" onClick={closeModal}>Close • إغلاق</button>
            </div>
          </div>
        </div>
      )}
      ...
      <style jsx global>{`
        ...
        *{ box-sizing:border-box }
        ...
        .modal{ width:min(440px,92vw); padding:16px; }
        .modalH{ margin:0 0 6px; color: var(--unrwa-dark); }
        .dropinWrap { margin:12px 0; }
        .rowBtns{ display:flex; flex-direction:column; gap:10px; margin-top:12px; }

        /* Primary button in modal */
        .payBtn { width:100%; height:44px; border-radius:10px; font-weight:800; }

        /* Close as link */
        .btnLink{
          background:transparent; border:0; padding:6px 4px; cursor:pointer;
          color:#355C82; text-decoration:underline; font-weight:600; width:max-content;
          align-self:center;
        }
      `}</style>
    </div>
