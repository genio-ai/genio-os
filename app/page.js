    ...
    <div className="page">
      <nav className="nav">
        ...
        <div className="navLinks">
          <a className="navLink" href="/chat" aria-label="Open Neo chat">Neo al-Ghazawi</a>
      ...
      {/* ===== styles ===== */}
      <style jsx global>{`
        :root{ --c1:#00B6FF; --c2:#00FFD1; --ink:#EAF7FF; --soft:#C7ECFF; --ink2:#D6F1FF; --panel:rgba(0,36,64,.82); }
        *{ box-sizing:border-box }
        body{ margin:0; background:
          radial-gradient(1200px 700px at 50% -15%, rgba(191,234,255,.24), transparent 60%),
        ...
        /* NAV */
        .nav{ position:sticky; top:0; z-index:50; display:flex; justify-content:space-between; align-items:center; padding:14px 20px;
              background:linear-gradient(180deg, rgba(0,90,160,.42), rgba(0,90,140,.18)); backdrop-filter:blur(8px); border-bottom:1px solid rgba(54,212,255,.35); }
        .brand{ font-weight:800; letter-spacing:1.2px; }
        .navLinks{ display:flex; gap:10px; align-items:center; }
        .navLink{ text-decoration:none; }
        .btnGhost, .navLink{ padding:10px 16px; border-radius:14px; color:var(--ink); background:transparent; border:1px solid rgba(92,200,245,.45); cursor:pointer; }
        ...
        @media (min-width: 992px){
          .grid{ display:grid; grid-template-columns: 1fr 420px; align-items:start; }
        }
        ...
        .card{ display:flex; justify-content:space-between; align-items:center;
               background:linear-gradient(180deg, rgba(0,36,64,.50), rgba(0,36,64,.36));
               border:1px solid rgba(54,212,255,.35); border-radius:18px; padding:14px 16px;
               box-shadow:0 10px 24px rgba(0,174,239,.18); }
        .enTitle{ font-weight:800; }
        .arTitle{ font-size:14px; color:var(--ink2); }
        .price{ font-size:14px; color:var(--soft); margin-top:6px; }
        .stepper{ display:flex; align-items:center; gap:8px; }
        .btnPill{ width:40px; height:40px; display:grid; place-items:center;
                  background:linear-gradient(135deg,#36D4FF,#00AEEF); color:#003A60; font-weight:900;
                  border:1px solid rgba(191,234,255,.28); border-radius:14px; cursor:pointer;
                  box-shadow:0 10px 24px rgba(31,216,255,.30); transition:transform .06s ease; }
        .btnPill:active{ transform:scale(.96); }
        .qty{ width:28px; text-align:center; font-weight:900; }
        ...
        /* ABOUT */
        .about{ margin:24px 0 12px; padding:16px; border-radius:18px;
                background:linear-gradient(180deg, rgba(0,36,64,.45), rgba(0,36,64,.28));
                border:1px solid rgba(54,212,255,.30); }
        .aboutTitle{ margin:0 0 6px; }
        .aboutP{ margin:0; color:var(--ink2); }

        /* STICKY BAR */
        .sticky{ position:sticky; bottom:0; z-index:40; display:flex; gap:12px; justify-content:space-between; align-items:center; padding:12px 16px;
                 backdrop-filter:blur(12px); background:rgba(0,58,96,.60); border-top:1px solid rgba(54,212,255,.35); }
        .btnPrimary{ background:linear-gradient(135deg, #36D4FF, #00AEEF); color:#003A60; font-weight:900; border:1px solid rgba(191,234,255,.25);
                     border-radius:14px; padding:12px 18px; box-shadow:0 12px 28px rgba(31,216,255,.35); cursor:pointer;
        ...
        .modal{ width:min(560px, 96vw); padding:18px; border-radius:20px; background:var(--panel);
                border:1px solid rgba(54,212,255,.38); box-shadow:0 16px 40px rgba(0,174,239,.45); color:var(--ink); }
