 ...
 * - No sample offers — only three equal buttons (Get My Link, Payout, Language)
 * - Contact button placed under the panel
 * - Modal appears only when Get My Link is clicked
 * - English default; Arabic shown only after pressing language button
 * - Client-only (no SSR / pre-render)
 */
...
export default function Page() {
  ...
  async function createPersonalLink() {
    ...
    setLoading(true);
    try {
      await new Promise((r) => setTimeout(r, 350)); // replace with API call in prod
      ...
      // create a generic personal link placeholder (replace with offer link when integrating)
      const personal = `${location.origin}/r/${encodeURIComponent(userKey)}`;

      // save as user's main link
    ...
    <main>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .statusBox{padding:12px;border-radius:12px;border:1px dashed #e8eef9;text-align:center;color:var(--muted);font-weight:700}
        ...
        @media (max-width:880px){ .actions{flex-direction:column} .btn{width:100%} .modal{margin:16px} }
      `}</style>
