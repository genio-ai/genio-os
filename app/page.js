...
export default function Page() {
  const [mode, setMode] = useState("daily");        // daily | luck | next
  ...
  useEffect(() => {
    // مباشرة نبدأ بجلسة قراءة بطاقة واحدة
    startReading("daily");
  ...
  function startReading(selectedMode) {
    ...
    setCards([]);
    setTimeout(() => {
      const drawn = drawUnique(countMap(selectedMode));
      setCards(drawn);
      setShuffling(false);
    }, 700); // إحساس خفيف بالخلط
  }

  function countMap(m) {
    return m === "daily" ? 1 : m === "luck" ? 3 : 5;
  }
    ...
    <main style={styles.page}>
      ...
      <section style={styles.room}>
        ...
        <p style={styles.subtitle}>
          Focus on your intention, breathe in, and receive your reading.
        </p>

        <div style={styles.controls}>
          <button
            style={btn(mode === "daily")}
        ...
        <div style={styles.table}>
          ...
          {!shuffling && cards.length > 0 && (
            <div style={styles.interpretation}>
              ...
              <p style={{ opacity: 0.9 }}>
                Ground yourself and reflect on what this spread highlights.
                Let clarity guide your next gentle step.
              </p>
              <div style={styles.manifestBox}>
                <div style={{ fontWeight: 600, marginBottom: 6 }}>Manifestation Tip</div>
                <p style={{ margin: 0 }}>
                  Breathe slowly for 30 seconds. State your intention in the present tense.
                  Take one practical action today aligned with it.
                </p>
              </div>
            </div>
          )}
...
const styles = {
  ...
  backdrop: {
    ...
    inset: 0,
    background:
      "radial-gradient(60% 55% at 50% 0%, rgba(255,255,255,0.06) 0%, rgba(11,12,42,0) 60%), linear-gradient(180deg, #0f1033 0%, #0b0c2a 60%)",
    pointerEvents: "none",
  },
  header: {
    position: "fixed",
  ...
  table: {
    background:
      "linear-gradient(180deg, rgba(19,20,60,0.9) 0%, rgba(11,12,42,0.95) 100%)",
  ...
  card: {
    ...
    padding: 12,
    background:
      "linear-gradient(180deg, rgba(18,19,52,0.8) 0%, rgba(12,13,44,0.9) 100%)",
  },
