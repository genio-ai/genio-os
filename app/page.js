...
function PayOverlay({ open, onClose, amount, onSuccess }) {
  ...
  // Load braintree-web (client + hosted-fields) with fallback
  async function loadBraintreeWeb() {
    const V = "3.110.0";
    async function inject(id, srcs) {
      ...
      throw lastErr || new Error("Failed to load Braintree scripts");
    }

    await inject("btw-client", [
      `https://js.braintreegateway.com/web/${V}/js/client.min.js`,
      `https://cdn.jsdelivr.net/npm/braintree-web@${V}/dist/client.min.js`,
    ]);

    await inject("btw-hosted", [
      `https://js.braintreegateway.com/web/${V}/js/hosted-fields.min.js`,
      `https://cdn.jsdelivr.net/npm/braintree-web@${V}/dist/hosted-fields.min.js`,
    ]);
  }
...
const hfStyle = {
  ...
  border:"1px solid #E3E8EF", borderRadius:10, padding:"0 12px", background:"#fff"
};
