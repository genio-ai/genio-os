        ...
        <ReviewStep
          ...
          onSubmit={async () => {
            try {
              ...
              // Dynamic import to avoid module/alias issues at runtime
              ...
              if (state.voice?.blob) {
                const file = new File(
                  [state.voice.blob],
                  state.voice.fileName ||
                    `voice-${Date.now()}.${extFromMime(state.voice.mime)}`,
                  {
                    type:
                      state.voice.mime ||
                      state.voice.blob.type ||
                      "audio/webm",
                  }
                );
...
function Banner({ type = "info", msg, onClose }) {
  ...
  return (
    <div
      className={`mb-4 border px-3 py-2 rounded ${color} flex items-start justify-between gap-3`}
    >
  ...
  return (
    <section className="space-y-4">
      ...
      <ul className="text-sm text-gray-300 list-disc pl-5 space-y-2">
        ...
        <li>
          Processing uses our private gateway with integrated AI/voice providers;
...
function PersonalityStep({ value, onChange, onNext, onBack, setError }) {
  ...
  const wordCount = useMemo(
    () => (about.trim() ? about.trim().split(/\s+/).length : 0),
    [about]
  );
  const aboutOk = wordCount >= 50 || about.trim().length >= 300;
  const aboutMsg = aboutOk
    ? "Looks good"
    : "Recommended ≈50+ words (you can continue)";
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        />
        <div
          className={`text-xs ${aboutOk ? "text-green-300" : "text-gray-400"}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="Tone">
          <select
            ...
            className="w-full rounded border border-gray-700 bg-black/20 p-2"
          >
...
function MediaCapture({ minSec, maxSec, value, onChange, onNext, onBack, setError }) {
    ...
    try {
      ...
      rec.onstop = async () => {
        try {
          ...
          const secondsRecorded = Math.max(
            1,
          ...
          if (secondsRecorded < minSec || secondsRecorded > maxSec) {
            setError?.(
              `Recommended length ${minSec}-${maxSec}s. Yours is ${secondsRecorded}s.`
            );
          }
        } catch (err) {
          onChange(null);
    ...
    el.addEventListener(
      ...
      () => {
        ...
        if (dur && (dur < minSec || dur > maxSec)) {
          setError?.(
            `Recommended length ${minSec}-${maxSec}s. Yours is ${dur}s.`
          );
        }
        onChange({
          blob: f,
  ...
  return (
    <section className="space-y-4">
      ...
      {supported ? (
        <div className="rounded border border-gray-700 p-3 space-y-3">
          ...
          <div className="flex items-center gap-3">
            {!recording ? (
              <button className="px-4 py-2 rounded bg-gray-200 text-black" onClick={startRec}>
                Record
              </button>
            ) : (
              <button className="px-4 py-2 rounded bg-red-500 text-white" onClick={stopRec}>
                Stop
              </button>
            )}
      ...
      <div className="flex justify-between pt-2">
        <button className="px-4 py-2 rounded border border-gray-700" onClick={onBack}>
          Back
        </button>
...
function ReviewStep({ personality, voice, busy, onBack, onSubmit }) {
  function line(label, value) {
    return (
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="text-sm text-gray-100 max-w-[70%] text-right">
          {String(value || "—")}
        </span>
      </div>
    );
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex justify-between pt-2">
        <button className="px-4 py-2 rounded border border-gray-700" onClick={onBack} disabled={busy}>
          Back
        </button>
