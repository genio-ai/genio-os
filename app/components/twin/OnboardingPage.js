 ...
 * Feature Flags
 * -------------------------------- */
const ENABLE_VIDEO = false; // voice only for now
        ...
        <ReviewStep
          ...
          onSubmit={async () => {
            try {
              ...
              // 1) get user id (browser supabase)
...
function MediaCapture({ minSec, maxSec, script, value, onChange, onNext, onBack, setError }) {
  ...
  const chunksRef = useRef([]);         // chunks buffer
  ...
  const [chunks, setChunks] = useState([]); // optional UI
  ...
  async function startRec() {
    try {
      ...
      // iOS/Safari-friendly: try with constraints, then fallback
      let stream;
      try {
      ...
      rec.onstop = async () => {
        try {
          ...
          await new Promise((r) => setTimeout(r, 40)); // flush last chunk (Safari)
          stopTracks();
      ...
      {value?.blob ? (
        <div className="rounded border border-gray-700 p-3 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button className="text-xs underline" onClick={clearMedia}>
              Clear
            </button>
          </div>
