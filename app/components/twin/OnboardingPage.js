...
/** ---------------------------
 *  Constants (relaxed for UX)
 * --------------------------*/
const VOICE_MIN_SEC = 20;     // recommended minimum (soft)
const VOICE_MAX_SEC = 120;    // recommended maximum (soft)
...
function pickSupportedMime(kind) {
  ...
  return kind === "voice" ? "audio/mp4" : "video/webm";
}
...
function PersonalityStep({ value, onChange, onNext, onBack, setError }) {
  ...
  const aboutOk = wordCount >= 50 || about.trim().length >= 300; // relaxed, soft warning
...
function VoiceStep({ minSec, maxSec, script, value, onChange, onNext, onBack, setError }) {
  return (
    <MediaCapture
      kind="voice"
      accept="audio/*,video/*"  // allow MOV from iPhone
      minSec={minSec}
      maxSec={maxSec}
...
function MediaCapture({ kind, accept, minSec, maxSec, help, value, onChange, onNext, onBack, setError }) {
  ...
  async function startRec() {
    try {
      ...
      // Choose best MIME (iOS prefers MP4)
      ...
      rec.onstop = () => {
        ...
        try {
          ...
          if (seconds < minSec || seconds > maxSec) {
            // soft warning only
            setError(`Recommended length ${minSec}-${maxSec}s. Yours is ${seconds}s.`);
          }
  ...
  function onFile(e) {
    ...
    // Choose element by actual file type so MOV works on the voice step
