 ...
 *  Constants (relaxed)
 * --------------------------*/
const VOICE_MIN_SEC = 20;     // was 60
const VOICE_MAX_SEC = 120;    // was 90
...
function PersonalityStep({ value, onChange, onNext, onBack, setError }) {
  ...
  const aboutOk = wordCount >= 50 || about.trim().length >= 300; // relaxed
...
function VoiceStep({ minSec, maxSec, script, value, onChange, onNext, onBack, setError }) {
  return (
    <MediaCapture
      kind="voice"
      accept="audio/*,video/*"          // accept MOV too
      minSec={minSec}
      maxSec={maxSec}
...
function MediaCapture({ kind, accept, minSec, maxSec, help, value, onChange, onNext, onBack, setError }) {
  ...
  function onFile(e) {
    ...
    // choose element by actual file type (so MOV works on voice step)
    ...
    el.addEventListener("error", () => {
      // accept anyway if metadata fails
