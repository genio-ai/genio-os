...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "ERROR":
      // important: do NOT force a default error message when payload is empty/null
        ...
        <ReviewStep
          ...
          onSubmit={async () => {
            try {
              ...
              // 3) (optional) video upload — not enabled here
...
function MediaCapture({ kind, accept, minSec, maxSec, help, value, onChange, onNext, onBack, setError }) {
  ...
  async function startRec() {
    try {
      setError?.(null); // important: clear without triggering default error
      setSeconds(0);
      setChunks([]);
      ...
      let autoStopTimer;
      rec.onstart = () => {
        autoStopTimer = setTimeout(() => { try { rec.stop(); } catch {} }, (kind === "voice" ? VOICE_MAX_SEC : VIDEO_MAX_SEC) * 1000);
      };
